<?php
    ...
    if((!empty($_GET['cpfVelho'])) and (!empty($_POST['cpf'])) and (!empty($_POST['nome'])) and (!empty($_POST['nascimento'])) and (!empty($_POST['sexo'])) and (!empty($_POST['estadoCivil'])) and (!empty($_POST['endereco']))) {
        $r = $db->prepare("SELECT cpf FROM cliente WHERE cpf=? AND cpf!=?");
        $r->execute(array($_POST['cpf'],$_GET['cpfVelho']));
        if($r->rowCount()>0) {$_SESSION['msgm'] = "<script>UIkit.notification({message: '<span uk-icon=\'icon: close\'></span> Cliente Cpf ".$_POST['cpf']." já existente', status: 'danger'})</script>"; header("location: advIndex.php");}
        else {
            $r = $db->prepare("UPDATE cliente SET cpf=?,nome=?,nascimento=?,sexo=?,estadoCivil=?,endereco=? WHERE cpf=?");
            $r->execute(array($_POST['cpf'],$_POST['nome'],$_POST['nascimento'],$_POST['sexo'],$_POST['estadoCivil'],$_POST['endereco'],$_GET['cpfVelho']));
            $_SESSION['msgm'] = "<script>UIkit.notification({message: '<span uk-icon=\'icon: check\'></span> Cliente ".$_POST['cpf']." atualizado', status: 'success'})</script>";
        ...
        <div>
            ...
            <form action="edCliente.php?cpfVelho=<?=base64_decode($_GET['cpf'])?>" method="post">
                <fieldset class="uk-fieldset">
                    <div class="uk-margin">
                        <input class="uk-input uk-form-width-medium" type="text" pattern="\d{3}\.\d{3}\.\d{3}-\d{2}" required name="cpf" placeholder="Cpf (xxx.xxx.xxx-xx)" value="<?=base64_decode($_GET['cpf'])?>">
                    </div>
                    <div class="uk-margin">
                    ...
                    <div class="uk-margin">
                        <label class="uk-form-label" for="formNascimento">Nascimento</label><br>
                        <input class="uk-input uk-form-width-small" type="date" id="formNascimento" max="<?=date('Y-m-d')?>" required name="nascimento" placeholder="Nascimento" value="<?=$nascimento?>">
                    </div>
                    <div class="uk-margin">
                        <label class="uk-form-label" for="selectSexo">Sexo</label><br>
                        <select class="uk-select uk-form-width-small" required id="selectSexo" name="sexo">
                    ...
                    <div class="uk-margin">
                        <label class="uk-form-label" for="selectEc">Estado civil</label><br>
                        <select class="uk-select uk-form-width-small" required id="selectEc" name="estadoCivil">
                    ...
                    <div class="uk-margin">
                        <input class="uk-input uk-form-width-large" type="text" required name="endereco" maxlength="150" placeholder="Endereço" style="text-transform: lowercase;" value="<?=$endereco?>">
                    </div>
