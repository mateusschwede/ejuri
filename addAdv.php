<?php
    ...
    $msgm = null;
    if((!empty($_POST['oab'])) and (!empty($_POST['nome'])) and (!empty($_POST['senha']))) {
        $r = $db->prepare("SELECT oab FROM advogado WHERE oab=?");
        $r->execute(array($_POST['oab']));
        if($r->rowCount()>0) {$msgm = "<script>UIkit.notification({message: '<span uk-icon=\'icon: close\'></span> Advogado Oab ".$_POST['oab']." já existente', status: 'danger'})</script>";}
        else {
            $r = $db->prepare("INSERT INTO advogado(oab,nome,senha) VALUES (?,?,?)");
            $r->execute(array($_POST['oab'],$_POST['nome'],$_POST['senha']));
            $_SESSION['msgm'] = "<script>UIkit.notification({message: '<span uk-icon=\'icon: check\'></span> Advogado ".$_POST['nome']." adicionado', status: 'success'})</script>";
...
<body>
    ...
    <div class="uk-child-width-expand@s" uk-grid>
        <div>
            <h2>Novo advogado</h2>
            <form action="addAdv.php" method="post">
                <fieldset class="uk-fieldset">
                    <div class="uk-margin">
                        <input class="uk-input uk-form-width-small" type="number" required name="oab" min=0 max=9999999999 placeholder="Oab">
                    </div>
                    <div class="uk-margin">
