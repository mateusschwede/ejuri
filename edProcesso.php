<?php
    ...
    if(!empty($_GET['id'])) {
        $r = $db->prepare("SELECT assunto,descricao,tipoAcao,valorHora FROM processo WHERE id=?");
    ...
    if((!empty($_GET['idVelho'])) and (!empty($_POST['assunto'])) and (!empty($_POST['descricao'])) and (!empty($_POST['tipoAcao'])) and (!empty($_POST['valorHora']))) {
        $r = $db->prepare("UPDATE processo SET assunto=?,descricao=?,tipoAcao=?,valorHora=? WHERE id=?");
        $r->execute(array($_POST['assunto'],$_POST['descricao'],$_POST['tipoAcao'],$_POST['valorHora'],$_GET['idVelho']));
        $_SESSION['msgm'] = "<script>UIkit.notification({message: '<span uk-icon=\'icon: check\'></span> Processo ".$_POST['id']." atualizado', status: 'success'})</script>";
...
<div class="uk-child-width-expand@s" uk-grid>
    ...
    <div class="uk-child-width-expand@s" uk-grid>
        <div>
            <h2>Processo <?=base64_decode($_GET['id'])?></h2>
            <form action="edProcesso.php?idVelho=<?=base64_decode($_GET['id'])?>" method="post">
                <fieldset class="uk-fieldset">
                    <div class="uk-margin">
                        <input class="uk-input uk-form-width-large" type="text" required name="assunto" maxlength="100" placeholder="Assunto" style="text-transform: lowercase;" value="<?=$assunto?>">
                    </div>
                    <div class="uk-margin">
                        <textarea class="uk-textarea uk-form-width-large" rows="5" required name="descricao" placeholder="Descrição" style="text-transform: lowercase; resize: none;"><?=$descricao?></textarea>
                    </div>
                    <div class="uk-margin">
                        ...
                        <select class="uk-select uk-form-width-small" required id="selectAcao" name="tipoAcao">
                            <option value="<?=$tipoAcao?>"><?=$tipoAcao?></option>
                    ...
                    <div class="uk-margin">
                        <input class="uk-input uk-form-width-large" type="number" required name="valorHora" lang="en" min=0 max=9999 step="0.01" placeholder="Valor hora" value="<?=number_format($valorHora,2,".","")?>">
                    </div>
